...
// Длина метки подлинности служебных кадров. Должна совпадать с
// crypto.ControlTagSize на стороне шлюза.
//
// Метка добавлена этой правкой формата. Прежде кадры ротации и
// подтверждения шли открытым текстом: тот, кто вклинился в соединение, мог
// вбросить подтверждение, не зная сеансового ключа. Шлюз применял ротацию,
// устройство нет, ключи расходились, и устройство в итоге отзывалось.
#define LACERT_CONTROL_TAG_SIZE      16
...
// Разделитель области для метки подлинности служебных кадров. Не даёт
// использовать эту метку там, где сеансовый ключ участвует в других
// вычислениях, и наоборот.
#define LACERT_SEP_CONTROL     "lacert-control-v1"
